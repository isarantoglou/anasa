...
<template>
  <div class="card-elevated p-6">
    ...
    <div class="max-h-100 space-y-1 overflow-y-auto pr-2">
      <div
        v-for="holiday in allHolidays"
        :key="holiday.date.toISOString()"
        class="group flex items-center justify-between rounded-xl px-4 py-3 transition-colors hover:bg-(--marble-100)"
      >
        <div class="min-w-0 flex-1">
          <div class="text-sm font-semibold text-(--marble-700)">{{ holiday.nameGreek }}</div>
        </div>
    ...
    <div v-if="weekendHolidays.length > 0" class="mt-4 border-t border-(--marble-200) pt-4">
      <button
      ...
      >
        <span class="flex items-center gap-2">
          ...
          <span class="text-sm text-(--warning-600)"
            >{{ weekendHolidays.length }} αργί{{
              weekendHolidays.length > 1 ? 'ες πέφτουν' : 'α πέφτει'
            }}
            Σαββατοκύριακο</span
          >
        </span>
        <svg
          class="h-4 w-4 text-(--marble-400) transition-transform duration-200"
      ...
      <!-- Expandable content -->
      <div class="expandable-content" :class="{ expanded: weekendHolidaysExpanded }">
        <div class="space-y-2 pt-3">
          <div
            v-for="holiday in weekendHolidays"
            :key="holiday.date.toISOString()"
            class="flex items-center justify-between rounded-lg bg-(--warning-100)/50 px-3 py-2"
          >
            <div>
