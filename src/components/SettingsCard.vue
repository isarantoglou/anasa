...
<template>
  <div class="card-elevated p-6">
    ...
    <div class="space-y-6">
      ...
      <div>
        <div class="mb-3 flex items-center justify-between">
          <label class="block text-sm font-semibold text-(--marble-600)"> Επιλογή Έτους </label>
          <button
            @click="emit('toggle-year-comparison')"
            class="flex items-center gap-1 text-xs font-semibold text-(--aegean-600) hover:text-(--aegean-700)"
          >
        ...
        <div class="flex items-center gap-2">
          ...
          <button
            ...
            class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-(--aegean-200) bg-(--aegean-50) transition-all hover:border-(--aegean-300) hover:bg-(--aegean-100) active:scale-95 sm:h-10 sm:w-10"
            aria-label="Προηγούμενο έτος"
          >
            <svg
              class="h-4 w-4 text-(--aegean-600) sm:h-5 sm:w-5"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
          ...
          <div class="relative flex-1 overflow-hidden">
            ...
            <div
              class="year-carousel-fade-left pointer-events-none absolute top-0 bottom-0 left-0 z-20 w-6 sm:w-8"
            ></div>
            <!-- Gradient fade on right -->
            <div
              class="year-carousel-fade-right pointer-events-none absolute top-0 right-0 bottom-0 z-20 w-6 sm:w-8"
            ></div>
            ...
            <div class="relative flex items-center justify-center gap-1 py-1.5">
              ...
              <button
                ...
                class="stat-number shrink-0 rounded-lg px-2 py-1.5 text-sm font-semibold whitespace-nowrap transition-colors duration-200 sm:px-3"
                :class="
                  index === 3 ? 'z-10 text-white' : 'text-(--marble-400) hover:text-(--marble-600)'
                "
              >
                {{ year }}
              </button>
      ...
      <div>
        ...
        <p v-if="showingFromToday" class="mt-2 text-xs text-(--marble-500)">
      ...
      <div>
        <div class="flex items-center justify-between">
          <label class="block text-sm font-semibold text-(--marble-600)"> Αγίου Πνεύματος </label>
          <button
        ...
        <p class="mt-2 text-xs text-(--marble-500)">
      ...
      <div>
        <div class="flex items-center justify-between">
          <label class="block text-sm font-semibold text-(--marble-600)"> Λειτουργία Γονέα </label>
          <button
        ...
        <p class="mt-2 text-xs text-(--marble-500)">
          {{
            parentMode
              ? 'Εμφάνιση ευκαιριών κατά τις σχολικές διακοπές'
              : 'Ενεργοποιήστε για οικογενειακό προγραμματισμό'
          }}
        </p>
        ...
        <div v-if="parentMode" class="mt-3 space-y-2">
          <div
            ...
            :key="schoolBreak.id"
            class="flex items-center justify-between rounded-lg border border-(--marble-200) bg-(--marble-100) p-2"
          >
            <div class="flex items-center gap-2">
              <span class="text-lg">{{ schoolBreak.icon }}</span>
              <span class="text-sm font-medium text-(--marble-700)">{{
                schoolBreak.nameGreek
              }}</span>
            </div>
            <span class="text-xs text-(--marble-500)">
              {{ format(schoolBreak.startDate, 'd MMM', { locale: el }) }} -
              {{ format(schoolBreak.endDate, 'd MMM', { locale: el }) }}
            </span>
          </div>
        </div>
      </div>
      ...
      <div>
        <label for="totalLeaveDays" class="mb-2 block text-sm font-semibold text-(--marble-600)">
          Συνολικές Ημέρες Άδειας
        </label>
        <p class="mb-3 text-xs text-(--marble-500)">Οι ημέρες άδειας που δικαιούστε ετησίως</p>
        <div class="flex items-center gap-3">
          <input
            id="totalLeaveDays"
            :value="totalAnnualLeaveDays"
            @input="
              emit('update:totalAnnualLeaveDays', Number(($event.target as HTMLInputElement).value))
            "
            type="number"
            min="1"
            max="50"
            class="input-elegant stat-number flex-1 text-center text-xl font-semibold text-(--aegean-700)"
          />
      ...
      <!-- Search Leave Days -->
      <div>
        <label for="searchDays" class="mb-2 block text-sm font-semibold text-(--marble-600)">
          Αναζήτηση με Ημέρες
        </label>
        <p class="mb-3 text-xs text-(--marble-500)">Πόσες ημέρες θέλετε να χρησιμοποιήσετε;</p>
        <input
          id="searchDays"
          ...
          min="1"
          max="30"
          class="input-elegant stat-number text-center text-2xl font-semibold text-(--aegean-700)"
        />
        ...
        <div
          v-if="searchExceedsRemaining"
          class="mt-3 rounded-lg border border-(--warning-300) bg-(--warning-100) p-3"
        >
          <div class="flex items-center gap-2 text-(--warning-700)">
            ...
            <span class="text-sm font-medium"
              >Αναζητάτε {{ searchLeaveDays }} ημέρες αλλά έχετε μόνο
              {{ remainingLeaveDays }} διαθέσιμες</span
            >
          </div>
        </div>
      </div>
    ...
    <!-- Year Stats -->
    <div class="mt-8 border-t border-(--marble-200) pt-6">
      <h3 class="mb-4 text-sm font-semibold tracking-wider text-(--marble-500) uppercase">
        {{ showingFromToday ? 'Υπόλοιπο ' + currentYear : 'Επισκόπηση Έτους' }}
      </h3>
      <div class="grid grid-cols-2 gap-3">
        <div class="rounded-xl border border-(--marble-200) bg-(--marble-100) p-4">
          <div class="text-xs font-semibold text-(--marble-500) uppercase">Εργάσιμες</div>
          <div class="stat-number mt-1 text-3xl font-semibold text-(--marble-700)">
            {{ stats.workdays }}
          </div>
        </div>
        <div class="rounded-xl border border-(--aegean-200) bg-(--aegean-50) p-4">
          <div class="text-xs font-semibold text-(--aegean-700) uppercase">Σαββατοκύριακα</div>
          <div class="stat-number mt-1 text-3xl font-semibold text-(--aegean-700)">
            {{ stats.weekendDays }}
          </div>
        </div>
        <div class="rounded-xl border border-(--terracotta-200) bg-(--terracotta-100) p-4">
          <div class="text-xs font-semibold text-(--terracotta-600) uppercase">Αργίες</div>
          <div class="stat-number mt-1 text-3xl font-semibold text-(--terracotta-700)">
            {{ stats.holidayDays }}
          </div>
        </div>
        <div class="rounded-xl border border-(--success-200) bg-(--success-50) p-4">
          ...
          <div class="stat-number mt-1 text-3xl font-semibold text-(--success-700)">
            {{ stats.freeDays }}
          </div>
        </div>
      </div>
